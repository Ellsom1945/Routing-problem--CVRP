...
def select(checknum,totallist1,totallist2,totallist3,carnumber):             #根据输入的车辆号码，从而得出该车的所在地点，返回它的下一步目标和目标的下标
    ...
    if(car[carnumber].buff=="到达需求地"):
        for i in range(1,len(totallist3[checknum])):             #计算当前需求地到需求地的效率
            if(coordinates1goods[totallist3[checknum][i][0]]<=car[carnumber].involume):               #如果需求地物资小于等于车载的物资，效率=需求地物资/距离
                coordinatesArrivable_one.append((totallist3[checknum][i][0],coordinates1goods[totallist3[checknum][i][0]]/totallist3[checknum][i][1]))
                #print(totallist3[checknum][i][0])
                #print(coordinates1goods[totallist3[checknum][i][0]])
            else:
                coordinatesArrivable_one.append((totallist3[checknum][i][0],car[carnumber].involume/totallist3[checknum][i][1]))         #checknum:需求地标号，i:遍历 【1】：距离
        ...
        for i in range(len(totallist1[checknum])):          #计算需求地到供应地的效率
            if (coordinates2goods[totallist1[checknum][i][0]] <= (car[carnumber].volume-car[carnumber].involume)):
                coordinatesArrivable_twe.append((totallist1[checknum][i][0],coordinates2goods[totallist1[checknum][i][0]]/totallist1[checknum][i][1]))
            else:
                coordinatesArrivable_twe.append((totallist1[checknum][i][0],(car[carnumber].volume-car[carnumber].involume)/totallist1[checknum][i][1]))
        ...
        if(t1[1]>0):                        #如果需求地效率>0,则继续前往，直到车上货物全部运完
            return ("需求地到需求地",t1[0])
        else:
            return ("需求地到供应地",t2[0])
    ...
    if (car[carnumber].buff == "到达供应地"):
        ...
        for i in range(len(totallist2)):                      #计算供应地到需求地的效率
            if(coordinates1goods[totallist2[checknum][i][0]]<=car[carnumber].involume):   #如果需求地物资小于车辆装载的货物，效率=需求地物资/距离
                coordinatesArrivable_twe.append((totallist2[checknum][i][0], coordinates1goods[totallist2[checknum][i][0]] / totallist2[checknum][i][1]))
            else:
                #print(coordinates1goods[totallist2[checknum][i][0]])
                #print(totallist2[checknum][i][1])
                coordinatesArrivable_twe.append((totallist2[checknum][i][0],car[carnumber].involume/totallist2[checknum][i][1]))
        #print("供应到供应的效率",coordinatesArrivable_one)
        #print("供应到需求的效率",coordinatesArrivable_twe)
        t2=maxselect(coordinatesArrivable_twe)
        return ("供应地到需求地",t2[0])

def check(checknum,totallist1,totallist2,totallist3,volume,carnumber):       #输入当前位置，车辆的号码，车辆当前运载量，返回目标点下标和距离
    ...
    op=select(car[carnumber].goto,totallist1,totallist2,totallist3,carnumber)
    s=op[1]
    min="bug"
    if(op[0]=="需求地到需求地"):
...
def transport():
    def yusong():
        op = check(car[i].goto,totallist1,totallist2,totallist3,car[i].involume,i)
        if(op[2]==" "):
            pass
        else:
            car[i].lat_x = car[i].x
